...
const postIntervenant = async (request, response) => {
  ...
  try {
    const searchedIntervenant = await Intervenant.findOne({
      NSS: intervenant.NSS,
    })
    ...
    response
      .status(500)
      .json({ error: "L'intervenant n'a pas pu être enregistré" })
  }
}
//get request
...
const getAllIntervenants = async (request, response) => {
  try {
    ...
    response
      .status(500)
      .json({ error: 'laffichage de tous les intervenats a échoué' })
  }
}
//delete request
...
const deleteIntervenant = async (request, response) => {
  try {
    ...
    response
      .status(500)
      .json({ error: "la suppression de l'intervenant a échoué" })
  }
}
//update intervenant
//I need request.params
...
const updateIntervenant = async (request, response) => {
  ...
  try {
    const updateIntervenant = await Intervenant.findByIdAndUpdate(
      id,
      newIntervenant,
      { new: true },
    )
...
const ProfileIntervenantToUser = async (req, res) => {
  ...
  const intervenantFound = await Intervenant.findById(id).populate('User', [
    'username,SelectedFile,email,NumSiret',
  ])
  res.status(200).json()
}
...
module.exports = {
  ...
  updateIntervenant,
  getOneIntervenant,
  ProfileIntervenantToUser,
  allIntervenantduclient,
}
