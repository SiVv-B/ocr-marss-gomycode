...
export const AllIntervUsers = () => async (dispatch) => {
  try {
    ...
    dispatch({
      type: GET_ALL_INTERVENANTSUSER,
      payload: response.data.intervenants,
    })
  } catch (error) {
    console.log(error)
  }
...
export const profileIntervUser = (id) => async (dispatch) => {
  try {
    ...
    console.log(
      'from Intervant Action one intervenant',
      response.data.intervenant,
    )
...
export const addIntervenant = (newIntervenant, navigate) => async (
  dispatch,
) => {
  try {
